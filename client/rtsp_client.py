...
class RTSPClient:
    def __init__(self, server_ip, server_port=8554, rtp_port=5004):
    ...
    def send_request(self, command):
        if command == 'SETUP':
            request = (
                f"SETUP rtsp://{self.server_ip}:{self.server_port}/video RTSP/1.0\r\n"
                f"CSeq: {self.cseq}\r\n"
            ...
            request = (
                f"{command} rtsp://{self.server_ip}:{self.server_port}/video RTSP/1.0\r\n"
                f"CSeq: {self.cseq}\r\n"
            )
            if self.session_id:
